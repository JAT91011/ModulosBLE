...
static void serialPacketParser( uint8 port, uint8 event);
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  {
    uint8 charValue1[SIMPLEPROFILE_CHAR1_LEN] = {' '};
    /*uint8 charValue2 = 2;
    uint8 charValue3 = 3;
    uint8 charValue4 = 4;
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:
      ...
      #endif // (defined HAL_LCD) && (HAL_LCD == TRUE)
      HalUARTWrite(HAL_UART_PORT_0, newValue, SIMPLEPROFILE_CHAR1_LEN);
    ...
    */
    ...
    case PACKET_DATA:
      ...
      if( separador == ';' )
      {
        ...
        uint8 guardar[SIMPLEPROFILE_CHAR2_LEN] = {' '};
        guardar[0]=pktId;
        ...
        for(; i < (SIMPLEPROFILE_CHAR2_LEN - 1); i++)
        {
          guardar[3+i] = ' ';
        }
        SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR2, SIMPLEPROFILE_CHAR2_LEN, &guardar );
